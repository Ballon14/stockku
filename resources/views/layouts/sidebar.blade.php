...
<aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-64 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out">
    ...
    <nav class="mt-4 px-3 space-y-1 overflow-y-auto h-[calc(100vh-180px)] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        ...
        <!-- Absensi Section -->
        @if(config('app.attendance_enabled') && $user->hasRole(['admin', 'karyawan', 'kasir']))
        <div class="pt-4">
        ...
        <a href="{{ route('settings.database') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 {{ Str::startsWith($currentRoute, 'settings.database') ? 'bg-indigo-600/20 text-indigo-300 border border-indigo-500/30 sidebar-active' : 'text-slate-300 hover:bg-slate-700/50 hover:text-white' }}">
