<x-guest-layout>
    ...
    <div class="text-center mb-8">
        ...
        <p class="mt-1 text-sm text-gray-500">Masuk untuk melanjutkan ke StockKu</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div>
            ...
            <div class="relative mt-1.5">
                ...
                <x-text-input id="email" class="block mt-0 w-full pl-11 py-2.5 rounded-xl border-gray-200 bg-gray-50/50 focus:bg-white transition" type="email" name="email" :value="old('email')" required autofocus autocomplete="username" placeholder="nama@perusahaan.com" />
            </div>
        ...
        <div>
            <div class="flex items-center justify-between">
                ...
                @if (Route::has('password.request'))
                    <a class="text-xs font-medium text-indigo-600 hover:text-indigo-500" href="{{ route('password.request') }}">
                        {{ __('Lupa kata sandi?') }}
                    </a>
                @endif
            </div>
            <div class="relative mt-1.5">
                ...
                <x-text-input id="password" class="block mt-0 w-full pl-11 py-2.5 rounded-xl border-gray-200 bg-gray-50/50 focus:bg-white transition"
                                type="password"
                                name="password"
        ...
        <div class="flex items-center">
            <label for="remember_me" class="flex items-center cursor-pointer select-none">
                <input id="remember_me" type="checkbox" class="rounded-md border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 focus:ring-offset-0" name="remember">
                <span class="ms-2 text-sm text-gray-600">{{ __('Ingat saya') }}</span>
            </label>
        </div>

        <div class="pt-1">
            <button type="submit" class="w-full inline-flex justify-center items-center gap-2 px-4 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 active:from-indigo-700 active:to-purple-700 rounded-xl font-semibold text-sm text-white shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/40 transition duration-150 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-white">
