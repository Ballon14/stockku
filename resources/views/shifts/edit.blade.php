    ...
    <form method="POST" action="{{ route('shifts.update', $shift) }}">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $shift->name) }}" required autofocus class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-200">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="start_time" value="{{ old('start_time', $shift->start_time->format('H:i')) }}" required placeholder="08:00" pattern="^([01]?\d|2[0-3]):[0-5]\d$" class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-200">
                ...
                <div>
                    ...
                    <input type="text" name="end_time" value="{{ old('end_time', $shift->end_time->format('H:i')) }}" required placeholder="17:00" pattern="^([01]?\d|2[0-3]):[0-5]\d$" class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-200">
        ...
        <div class="mt-6 flex justify-end gap-3">
            ...
            <button type="submit" class="px-4 py-2.5 rounded-xl bg-indigo-600 text-white font-semibold hover:bg-indigo-700 shadow-lg shadow-indigo-500/30 transition-colors">Perbarui Shift</button>
        </div>
    </form>
</div>
</x-app-layout>
