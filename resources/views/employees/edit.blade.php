        ...
        <form method="POST" action="{{ route('employees.update', $employee) }}">@csrf @method('PUT')
            <div class="space-y-4">
                ...
                <div class="space-y-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <div><label class="block text-sm font-medium text-slate-700 mb-1">Email Akun <span class="text-red-500">*</span></label><input type="email" name="user_email" value="{{ old('user_email', $employee->user?->email) }}" class="w-full rounded-xl border-slate-200 text-sm" required></div>
                    <div><label class="block text-sm font-medium text-slate-700 mb-1">Password Baru {{ $employee->user ? '(kosongkan jika tidak diubah)' : '<span class="text-red-500">*</span>' }}</label><input type="password" name="user_password" class="w-full rounded-xl border-slate-200 text-sm" {{ $employee->user ? '' : 'required' }}></div>
                    <div><label class="block text-sm font-medium text-slate-700 mb-1">Role <span class="text-red-500">*</span></label>
                        <select name="user_role" class="w-full rounded-xl border-slate-200 text-sm" required>
                            <option value="karyawan" {{ ($employee->user?->roles->first()?->name ?? 'karyawan') == 'karyawan' ? 'selected' : '' }}>Karyawan</option>
