        ...
        <div class="p-4">
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-1">
                    ...
                    <form method="POST" action="{{ route('employees.toggle-active', $emp) }}" onsubmit="return confirmForm(this, 'Yakin ingin {{ $emp->is_active ? 'menonaktifkan' : 'mengaktifkan' }} akun {{ $emp->nama }}?', { title: '{{ $emp->is_active ? 'Nonaktifkan Akun' : 'Aktifkan Akun' }}', confirmText: 'Ya, {{ $emp->is_active ? 'Nonaktifkan' : 'Aktifkan' }}', danger: {{ $emp->is_active ? 'true' : 'false' }} })">
                        @csrf
                        <button class="p-1.5 rounded-lg text-slate-400 hover:bg-{{ $emp->is_active ? 'red' : 'emerald' }}-50 hover:text-{{ $emp->is_active ? 'red' : 'emerald' }}-600" title="{{ $emp->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
            ...
            @if($emp->user && !$emp->is_active)
            <p class="text-xs text-red-600 font-semibold mt-2"><i class="fa-solid fa-ban me-1"></i>Akun nonaktif</p>
            @endif
        </div>
        @empty
            ...
            <tbody>
                ...
                <tr class="border-b border-slate-50 hover:bg-slate-50/50">
                    ...
                    <td class="py-3 px-4 text-center">
                        @if($emp->user)
                        <span class="px-2 py-0.5 rounded-full text-xs font-semibold {{ $emp->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700' }}">{{ $emp->user->roles->first()->name ?? 'user' }}</span>
                        @if(!$emp->is_active)
                        <span class="block text-xs text-red-600 font-semibold mt-1">Nonaktif</span>
                        @endif
                        @else
                    ...
                    <td class="py-3 px-4 text-center">
                        <div class="flex items-center justify-center gap-1">
                            ...
                            <form method="POST" action="{{ route('employees.toggle-active', $emp) }}" onsubmit="return confirmForm(this, 'Yakin ingin {{ $emp->is_active ? 'menonaktifkan' : 'mengaktifkan' }} akun {{ $emp->nama }}?', { title: '{{ $emp->is_active ? 'Nonaktifkan Akun' : 'Aktifkan Akun' }}', confirmText: 'Ya, {{ $emp->is_active ? 'Nonaktifkan' : 'Aktifkan' }}', danger: {{ $emp->is_active ? 'true' : 'false' }} })">
                                @csrf
                                <button class="p-1.5 rounded-lg text-slate-400 hover:bg-{{ $emp->is_active ? 'red' : 'emerald' }}-50 hover:text-{{ $emp->is_active ? 'red' : 'emerald' }}-600" title="{{ $emp->is_active ? 'Nonaktifkan' : 'Aktifkan' }}"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg></button>
                            </form>
                            <form method="POST" action="{{ route('employees.destroy', $emp) }}" onsubmit="return confirmForm(this, 'Yakin hapus karyawan ini?')">@csrf @method('DELETE')<button class="p-1.5 rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-600"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button></form>
                        </div>
                    </td>
